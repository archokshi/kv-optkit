# ====================================================================
# KV-OptKit Phase 2 Demo: Autopilot
# ====================================================================
#
# This script demonstrates the Phase 2 autopilot functionality. The system
# automatically creates and executes an optimization plan.
...
python examples/demo_cli.py submit --seq seq_3 --tokens 3000
...
Write-Host "`nStep 4: Running Autopilot to create and execute optimization plan..." -ForegroundColor Green
$body = @{
  target_hbm_util = 0.003 # Target lower than current HBM
  max_actions     = 5
  dry_run         = $false
} | ConvertTo-Json

$plan = Invoke-RestMethod -Method Post `
  -Uri http://localhost:9001/autopilot/plan `
...
Write-Host "`nStep 5: Getting final telemetry to show optimization results..." -ForegroundColor Green
...
Write-Host "HBM Utilization was reduced by $reduction (a $($reduction_percent.ToString("F2"))% improvement)." -ForegroundColor Yellow
